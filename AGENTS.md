...
- Property-based tests: use `bolero` crate for structural invariants (format, determinism, commutativity, roundtrip). Prefer property tests over exhaustive unit tests when validating properties that should hold for all inputs.
...
- Create commits at good stopping points (e.g., after completing a logical unit of work, a task phase, or before switching context).
- Steering and planning files (`.kiro/`, `AGENTS.md`, `FEEDBACK.md`, `CHANGES.md`) should be committed separately from implementation code. When preparing a PR, squash into two commits:
  1. The first commit contains only PR-ready implementation, tests, and config changes.
  2. The second commit contains steering/planning files to preserve history without polluting the PR.
- Follow the project's conventional commit format: `type: description` (e.g., `feat:`, `chore:`, `build(deps):`).
