...
Key terms and concepts in Duvet's internal data model:

- **Annotation**: A special comment in source code (or entry in a TOML file) that links implementation to a specification. Contains a target (what spec it references), a quote (the spec text it covers), and a type describing the nature of the link. Defined in `annotation.rs`.

- **AnnotationType**: Classifies what an annotation represents:
  - **Spec**: Defines a requirement extracted from a specification (created by `duvet extract` or `[[specification]]` config). These are the "things that need coverage."
  - **Citation** (default): An implementation claim — code that implements a requirement. Also called "implementation" in config and snapshot output.
  - **Test**: A test that verifies a requirement.
  - **Implication**: The requirement is satisfied as a logical consequence of the design (counts as full coverage).
  - **Exception**: The requirement is intentionally not met (counts as full coverage).
  - **Todo**: Marks a requirement as known-uncovered, needing future work.

- **AnnotationLevel**: The RFC 2119 requirement strength: `Auto` (inferred), `May`, `Should`, or `Must`. Determines how strictly coverage is enforced.

- **Target**: A specification document that annotations reference. Combines a `TargetPath` (URL or local file path) with a `Format` hint. For example, `https://www.rfc-editor.org/rfc/rfc9000` is a target. Defined in `target.rs`. Not to be confused with the `target` *field* on an annotation, which is the raw string (path + optional `#section` fragment).

- **TargetPath**: The resolved location of a specification — either a `Url` (for remote RFCs) or a `Path` (for local files). Handles URL canonicalization (e.g., normalizing various IETF URL formats to a single canonical form).

- **Specification**: A parsed specification document containing sections and lines of text. Duvet supports IETF RFC format and Markdown. Defined in `specification.rs`.

- **Section**: A named subdivision of a specification (e.g., `#section-2.1`), identified by an `id` and containing lines of text that may be requirements.

- **Reference**: The result of matching an annotation's quote to actual text in a specification. Links an `Annotation` to a byte range (`Slice`) in the spec where the quoted text was found. Defined in `reference.rs`.

- **AnnotationSet**: The complete set of all annotations (`BTreeSet<Arc<Annotation>>`), sorted and deduplicated.

- **ReportResult**: The top-level structure passed to report generators. Contains all targets (each with their `TargetReport`), all annotations, and link templates. Defined in `report.rs`.

- **TargetReport**: Per-specification report data: the parsed specification, all references matched against it, coverage statuses, and flags for whether citations/tests are required.

- **StatusMap**: Coverage statistics per Spec-type annotation, keyed by annotation ID. Each entry is a `Spec` struct with byte-offset counts for each coverage type.

- **Coverage**: A Spec annotation is "covered" when other annotations (Citation, Test, Implication, Exception) have quotes that overlap the same byte offsets in the specification text. Exception and Implication provide full coverage; Citation and Test each independently satisfy the remaining coverage requirement.

- **Format**: The specification document format: `Auto` (detect from content), `Ietf` (RFC text format), or `Markdown`.

- **Slice**: A reference to a substring within a source file, carrying line/offset metadata. Used throughout for zero-copy text handling.

- **blob_link**: A URL template for linking back to source files in a repository (e.g., GitHub blob URLs). Can be set globally or per-source.
